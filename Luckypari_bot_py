...
async def partner_callback(
    ...
    if action == "ai":
        ...
        await query.message.reply_text(
            (
                "🤖 <b>AI yordamchi</b>\n\n"
                "FTD, trafik, konversiya, kreativ, reklama "
                "yoki auditoriyani qiziqtirish bo'yicha "
                "savolingizni yozing.\n\n"
...
async def contact_start_callback(
    ...
    await query.message.reply_text(
        (
            ...
            "🔒 Suhbat tugamaguncha boshqa bo'limlar "
            "vaqtincha yopiq bo'ladi."
        ),
        parse_mode=ParseMode.HTML,
        reply_markup=active_chat_keyboard(),
    )
...
async def registration_model_selected(
    ...
    registration["model"] = query.data.split(
        ":",
        1,
    )[1]
...
async def registration_username_received(
    ...
    if not USERNAME_RE.match(username_value):
        await update.effective_message.reply_text(
            (
...
async def user_message_handler(
    ...
    # --------------------------------------------------------
    # BANNER SO'ROVINI TOMMY'GA YUBORISH
    # --------------------------------------------------------
    ...
    if banner_request:
        ...
        category = banner_request.get(
            "category",
            "unknown",
        )

        banner_type = banner_request.get(
            "type",
            "unknown",
        )
        ...
        banner_type_name = banner_type_names.get(
            banner_type,
            banner_type.upper(),
        )
        ...
        admin_text = (
            ...
            f"🖼 <b>Banner turi:</b> "
            f"{escape(banner_type_name)}\n"

            f"🔑 <b>Promo-kod:</b> "
            f"<code>{escape(promo)}</code>\n"

            f"🔢 <b>So'rov №:</b> "
            f"<code>{ticket_id}</code>\n\n"

            "━━━━━━━━━━━━━━━━━━\n"
            "↩️ <b>Tayyor bannerni ushbu xabarga Reply qilib "
            "rasm yoki fayl shaklida yuboring.</b>"
        )
        ...
        try:
            ...
            user_session["locked"] = True
            user_session["category"] = "banner"
            user_session["ticket_id"] = ticket_id
            user_session["waiting_first_message"] = False

            await message.reply_text(
                (
                    "✅ <b>Banner so'rovingiz Tommy'ga yuborildi.</b>\n\n"

                    f"🎯 Yo'nalish: {escape(category_name)}\n"
                    f"🖼 Banner turi: {escape(banner_type_name)}\n"
                    f"🔑 Promo-kod: <code>{escape(promo)}</code>\n\n"

                    "⏳ Iltimos, kuting. Tommy tayyor bannerni "
                    "shu chat orqali yuboradi.\n\n"

                    "🔒 Banner tayyor bo'lguncha boshqa "
                    "bo'limlar vaqtincha yopiladi."
                ),
                parse_mode=ParseMode.HTML,
                reply_markup=active_chat_keyboard(),
            )
            ...
            logger.exception(
                "Banner so'rovi admin'ga yuborilmadi: %s",
                error,
            )

            await message.reply_text(
                (
                    "⚠️ Banner so'rovini yuborishda "
    ...
    if not link:
        await message.reply_text(
            (
                ...
                "murojaat, banner so'rovi yoki yangi hamkor "
    ...
    try:
        if message.text:
            ...
            await context.bot.send_photo(
                ...
                caption=(
                    "🖼 <b>Tommy sizga tayyor banner yubordi.</b>\n\n"
                    + escape(
                        message.caption or ""
                    )
                ),
            ...
            await context.bot.send_document(
                ...
                caption=(
                    "📎 <b>Tommy sizga tayyor fayl yubordi.</b>\n\n"
                    + escape(
                        message.caption or ""
                    )
                ),
